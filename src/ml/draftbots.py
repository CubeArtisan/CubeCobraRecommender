...
class DraftBot(tf.keras.models.Model):
    def __init__(self, num_cards, batch_size, embed_dims=64, num_heads=16, summary_period=1024,
        ...
        self.oracle_weights = self.add_weight('oracle_weights', shape=(3, 15, 6), initializer=tf.constant_initializer(1), trainable=True)
    ...
    def call(self, inputs, training=False, mask=None):
        ...
        oracle_weights_orig = tf.math.softplus(20 * self.oracle_weights)
        ...
        pick_synergy_scores = (tf.einsum('ble,bce->bcl', pool_embed, normalized_in_pack_embeds, name='pick_synergy_scores') + tf.constant(1, dtype=self.compute_dtype)) / tf.constant(2, dtype=self.compute_dtype)
        # The rating oracle for each card is its rating times its casting probability.
        rating_scores = tf.expand_dims(in_pack_ratings, 2) + tf.zeros_like(pick_synergy_scores)
        ...
        # internal_synergy_scores = (tf.einsum('ble,bpe->bpl', pool_embed, normalized_picked_embeds,name='internal_synergies_scores') + tf.constant(1, dtype=self.compute_dtype)) / tf.constant(2, dtype=self.compute_dtype)
        ...
        # colors_scores = tf.expand_dims(picked_ratings, 2) + tf.zeros_like(internal_synergy_scores)
        # colors_scores = tf.zeros_like(internal_synergy_scores)
        ...
        seen_synergy_scores = (tf.einsum('ble,bse->bsl',pool_embed, normalized_seen_embeds, name='seen_synergies_scores') + tf.constant(1, dtype=self.compute_dtype)) / tf.constant(2, dtype=self.compute_dtype)
        ...
        openness_scores = tf.expand_dims(seen_ratings, 2) + tf.zeros_like(seen_synergy_scores)
        # openness_scores = tf.zeros_like(seen_synergy_scores)
        # Combine the oracle scores linearly according to the oracle weights to get a score for every card/land-configuration pair.
        in_pack_scores = tf.einsum('bclo,bo,blc->bcl', tf.stack([rating_scores, pick_synergy_scores], axis=3),
                                   oracle_weights[:, :2], prob_in_packs)
        # picked_scores = tf.einsum('bplo,bo,blp,b->bl', tf.stack([colors_scores, internal_synergy_scores], axis=3),
        #                           oracle_weights[:, 2:4], prob_pickeds, tf.constant(1, dtype=self.compute_dtype) / tf.math.maximum(tf.constant(1, dtype=self.compute_dtype), picked_counts))
        seen_scores = tf.einsum('bslo,bo,bls,b->bl', tf.stack([openness_scores, seen_synergy_scores], axis=3),
                                oracle_weights[:, 4:6], prob_seens, tf.constant(1, dtype=self.compute_dtype) / tf.math.maximum(tf.constant(1, dtype=self.compute_dtype), seen_counts))
        ...
        exp_scores = tf.reduce_sum(tf.math.exp(scores - max_scores), 2)
        # # Since the first operation of softmax is exp and the last of logsumexp is log we can combine them into a no-op.
        choice_probs = exp_scores / tf.math.reduce_sum(exp_scores, 1, keepdims=True)
        ...
        if training and tf.summary.experimental.get_step() % self.summary_period == 0:
            ...
            temperatures = tf.math.reduce_sum(oracle_weights_orig[:, :, 0:2], axis=2) + tf.math.reduce_sum(oracle_weights_orig[:, :, 3:6], 2)
            ...
            with tf.xla.experimental.jit_scope(compile_ops=False):
                ...
                for name, idx, values, expand in (('rating', 0, rating_scores, False), ('pick_synergy', 1, rating_scores, False),
    ...
    def calculate_loss(self, data, training=False):
        ...
        oracle_weights = tf.math.softplus(20 * self.oracle_weights)
